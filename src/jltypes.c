                                   ...
                                   cenv_t *penv, cenv_t *eqc, variance_t var)
{
    ...
    jl_svec_t *t = jl_alloc_sv(jl_svec_len(a->types));
    JL_GC_PUSH1(&t);
                                 ...
                                 cenv_t *penv, cenv_t *eqc, variance_t var)
{
    ...
    jl_svec_t *p = jl_alloc_sv(jl_svec_len(a->parameters));
    JL_GC_PUSH1(&p);
    jl_value_t *ti;
    size_t i;
...
static int tuple_to_Type(jl_datatype_t *a, jl_value_t **ptemp)
{
    ...
    int alen = jl_svec_len(ap);
    *ptemp = jl_alloc_sv(alen);
    int i;
    for(i=0; i < alen; i++) {
        jl_value_t *el = jl_svecref(ap, i);
        if (jl_is_type_type(el)) {
            jl_svecset(*ptemp, i, jl_tparam0(el));
        }
        else {
            *ptemp = NULL;
            return 0;
                                ...
                                jl_typestack_t *stack, int check)
{
    ...
    if (jl_is_uniontype(t)) {
        jl_svec_t *p = ((jl_uniontype_t*)t)->types;
        size_t lp = jl_svec_len(p);
        jl_svec_t *np = jl_alloc_svec(lp);
        JL_GC_PUSH1(&np);
        for(i=0; i < lp; i++) {
            jl_svecset(np, i, (jl_value_t*)inst_type_w_(jl_svecref(p,i), env, n, stack, 1));
        }
        jl_value_t *res = (jl_value_t*)jl_type_union(np);
        JL_GC_POP();
        return res;
    }
    if (!jl_is_datatype(t))
